...
- Made the app usable on mobile (viewport < 768px) with the same routes and no parallel component tree: a fixed bottom tab bar (Today, Calendar, Tasks, Focus, and Mail) plus a mobile top bar (app name, date, profile menu), and a new mobile-first Today screen — overdue tasks on top, then today's events and scheduled tasks in chronological order with tap-to-complete checkboxes and a floating quick-add that opens as a bottom sheet. Added a shared house-format bottom-sheet primitive, a `useIsMobile` breakpoint hook, a "Space is best on desktop" placeholder in place of the pan/zoom canvas on small screens, and a subtle "Add Needt to your home screen" install banner that only appears from the second visit. Aligned the PWA manifest theme color with the app chrome (`#1B1D1E`). (Calendar day-view swipe, Kanban horizontal snap, long-press action sheet, and pull-to-refresh are marked //todo for a follow-up.)
